...
from app.schemas.github import GitHubAuthUrl, GitHubRepository, GitHubSuggestedSkill, GitHubSyncResponse, GitHubStatus
...
async def github_callback(code: str, request: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    integration = db.query(GitHubIntegration).filter(GitHubIntegration.user_id == current_user.id).first()
    if integration:
        integration.github_username = github_username
        integration.last_sync = None
    else:
        integration = GitHubIntegration(user_id=current_user.id, github_username=github_username)
        db.add(integration)
    db.commit()
