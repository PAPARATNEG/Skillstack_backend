...
def enrich_skill(skill: Skill):
    ...
    return {**skill.__dict__, "status": status}
...
def get_skills(
    ...
    status: Optional[str] = Query(None, pattern="^(planned|in_progress|learned|expert)$"),
...
def create_skill(
    ...
    existing = (
        db.query(Skill)
        .filter(Skill.skill_group_id == group_id, Skill.name == data.name)
        .first()
    )
    if existing:
        raise HTTPException(400, "Skill with this name already exists in the group")

    skill = Skill(**data.model_dump(), user_id=current_user.id, skill_group_id=group_id)
    db.add(skill)
    db.commit()
    db.refresh(skill)
