...
def get_goals(
    ...
    goals = query.options(
        joinedload(Goal.skill_group).joinedload(SkillGroup.skills)
    ).all()
    result = []
    for g in goals:
...
def create_goal(
    ...
    group = (
        db.query(SkillGroup)
        .filter(
            SkillGroup.id == data.skill_group_id, SkillGroup.user_id == current_user.id
        )
        .first()
    )
    if not group:
    ...
    active = (
        db.query(Goal)
        .filter(Goal.skill_group_id == data.skill_group_id, Goal.is_completed == False)
        .first()
    )
    if active:
...
def get_goal(
    ...
    goal = (
        ...
        .options(joinedload(Goal.skill_group).joinedload(SkillGroup.skills))
        .first()
    )
    if not goal:
...
def update_goal(
    ...
    goal = (
        db.query(Goal)
        .filter(Goal.id == goal_id, Goal.user_id == current_user.id)
        .options(joinedload(Goal.skill_group))
        .first()
    )
    if not goal:
        raise HTTPException(404, "Goal not found")
    ...
    goal.skill_group.skills = (
        db.query(Skill).filter(Skill.skill_group_id == goal.skill_group_id).all()
    )
    progress = calculate_goal_progress(goal)
    return GoalOut.model_validate({**goal.__dict__, **progress})
