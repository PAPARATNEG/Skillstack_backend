...
class SkillConnection(Base, UUIDMixin, TimestampMixin):
    ...
    from_skill = relationship(
        "Skill", foreign_keys=[from_skill_id], back_populates="outgoing_connections"
    )
    to_skill = relationship(
        "Skill", foreign_keys=[to_skill_id], back_populates="incoming_connections"
    )

    __table_args__ = (
        CheckConstraint("from_skill_id != to_skill_id", name="different_skills"),
        UniqueConstraint(
            "from_skill_id", "to_skill_id", name="unique_directed_connection"
        ),
    )
