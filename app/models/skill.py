...
class Skill(Base, UUIDMixin, TimestampMixin):
    ...
    skill_group_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("skill_groups.id", ondelete="CASCADE"), nullable=False
    )
    ...
    node = relationship(
        "SkillNode", back_populates="skill", uselist=False, cascade="all, delete-orphan"
    )
    outgoing_connections = relationship(
        "SkillConnection",
        foreign_keys="SkillConnection.from_skill_id",
        back_populates="from_skill",
        cascade="all, delete-orphan",
    )
    incoming_connections = relationship(
        "SkillConnection",
        foreign_keys="SkillConnection.to_skill_id",
